...
lastDigit :: Int -> Int 
lastDigit n = if n > 0 then mod n 10 else abs(rem n 10)
...
{-
    | (x == minimum [x,y,z] || x == maximum [x,y,z]) && (y == minimum [x,y,z] || y == maximum [x,y,z]) = z 
    | (x == minimum [x,y,z] || x == maximum [x,y,z]) && (z == minimum [x,y,z] || z == maximum [x,y,z]) = y
    | otherwise = x
-}
{- |
=⚔️= Task 8
...
isVowel :: Char -> Bool
isVowel x 
    | toLower x == 'a' || toLower x == 'e' || toLower x == 'i' || toLower x == 'o' || toLower x == 'u' = True 
    | otherwise = False 
...
-}
sumLast2 :: Int -> Int 
--sumLast2 n = let resRemainder = divMod n 10 in snd resRemainder + snd (divMod (fst resRemainder) 10 )
sumLast2 n = 
      if n > 0 then 
        let resRemainder = divMod n 10 in snd resRemainder + snd (divMod (fst resRemainder) 10 )
        else 
            let resRemainder = quotRem n 10 in abs(snd resRemainder + snd (quotRem (fst resRemainder) 10 ))
{- |
=💣= Task 10*
...
-}
firstDigit :: Int -> Int
firstDigit n = if fst (divMod n 10) == 0 then snd (divMod n 10) else firstDigit (fst (divMod n 10))
