...
-}
subList :: Int -> Int -> [a] -> [a]
subList start end list = if (start < 0 || end < 0) then [] else take (end - start + 1) ( drop start list)
...
takeEven l = go [] l
  where
    go :: [Int] -> [Int] -> [Int]
    go acc [] = acc
    go acc [even] = reverse (even : acc)
    go acc [even, _] = reverse (even : acc)
    go acc (even: _: xs) = go (even: acc) xs 
...
-- contains val l = filter (elem val) l 
...
-}
rotate :: Int -> [Int] -> [Int]
rotate n l = if (length l) > 0 then drop n (take ((length l) + n) (cycle l)) else []
